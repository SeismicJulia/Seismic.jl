...
function SeisCopy(in::ASCIIString,out::ASCIIString)
	...
	filename_headers_in = success(`grep "headers=" $in`) ? chomp(readall(`grep "headers=" $in` |> `tail -1` |> `awk '{print substr($1,10,length($1)-10) }' `)) : "NULL"
	...
	esize = int(chomp(readall(`grep "esize" $in` |> `tail -1` |> `awk '{print substr($1,7,length($1))}'` )))
