...
function SegyToSeis(filename_in,filename_out,param=Dict())
    ...
    if (format=="su" || format=="rsf")
        ...
        file_hsize = 3600
	# add commands here to read text and binary headers and write them out to filename_out.thead and filename_out.bhead
        stream     = open(filename_in)
        position = 3200
        seek(stream, position)
        fh         = GrabFileHeader(stream)
        ntfh       = fh.netfh
        println("the number of extra text file is $ntfh")
        if ntfh == -1
    ...
    if (format=="segy" || format=="su")
      ...
      for j=1:nx
		  ...
		  SeisWrite(filename_out,d,h_seis,j)
      end
      close(stream)
      ...
      h_seis = Array(Header,1)
      h_seis[1] = InitSeisHeader()
      h_seis[1].n1 = nt
      h_seis[1].d1 = dt
      for j=1:nx
          position = file_hsize + total*(j-1)*4
    	  seek(stream,position)
    	  d[:] = read(stream,Float32,nt)
          if (swap_bytes==true)
    	     d = bswap_vector(d)
