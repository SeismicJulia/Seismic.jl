...
function ShotProfileWEM(m,d,param)
	...
	if (nangx != 1 || nangy != 1)
		for ishot = 1 : nshot
			SeisWindow(angx,shot_list[ishot].angx,["sx","sy"],[sx[ishot],sy[ishot]],[sx[ishot],sy[ishot]])
			SeisWindow(angy,shot_list[ishot].angy,["sx","sy"],[sx[ishot],sy[ishot]],[sx[ishot],sy[ishot]])
		end
	end

	if (adj == true)
		for ishot = 1 : nshot
			SeisWindow(d,shot_list[ishot].d,["sx","sy","gx","gy"],[sx[ishot],sy[ishot],min_gx,min_gy],[sx[ishot],sy[ishot],max_gx,max_gy])
		end
		...
		for ishot = 1 : nshot
			...
			m_shot,h_shot,status  = SeisRead(shot_list[ishot].m)
			if (nangx != 1 || nangy != 1)
				angx_shot,h_ang,status = SeisRead(shot_list[ishot].angx)
				angy_shot,h_ang,status = SeisRead(shot_list[ishot].angy)
			else
				angx_shot = 0.*m_shot
				angy_shot = 0.*m_shot
			end	
		...
		for ishot = 1 : nshot
			...
			max_imxa = max_imx < min_imxa + nhx - 1 ? max_imx : min_imxa + nhx - 1
			...
			max_imya = max_imy < min_imya + nhy - 1 ? max_imy : min_imya + nhy - 1
		...
		j = 1
		for ishot = 1 : nshot
			d_shot,h_shot,status = SeisRead(shot_list[ishot].d)
			SeisWrite(d,d_shot,h_shot,j)
			j += size(d_shot,2)
			rm(join([shot_list[ishot].d ".seisd"]))
			rm(join([shot_list[ishot].d ".seish"]))
			rm(join([shot_list[ishot].m ".seisd"]))
