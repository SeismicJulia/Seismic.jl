...
function ComputeAngles(angx,angy,param)
	...
	j = 1
	for ishot = 1 : nshot
		angx_shot,h_shot,status = SeisRead(shot_list[ishot].angx)
		angy_shot,h_shot,status = SeisRead(shot_list[ishot].angy)
		SeisWrite(angx,angx_shot,h_shot,j)
		SeisWrite(angy,angy_shot,h_shot,j)
		j += size(angx_shot,2)
